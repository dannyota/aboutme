...
const registryPath = join(here, "public-roots.v4.json");
...
test("the v4 registry is closed and authority ordered", async () => {
  ...
  assert.throws(() =>
    parsePublicRoots(Buffer.from(JSON.stringify({ ...registry, version: 3 }))),
  );
  assert.throws(() =>
    parsePublicRoots(
      Buffer.from(
