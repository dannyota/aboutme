...
for file in \
  packages/publicroots/public-roots.v9.json \
...
run)
  ...
  local | production)
    ...
    ;;
  transport | editor | public | password-auth | mcp | entry | publish | exports | privacy | sample-start | linkedin | second-factor | second-factor-disabled | mcp-sdk)
    previous_index=$(($# - 1))
...
grep -Fq 'mode must be auth, transport, editor, public, password-auth, mcp, entry, publish, exports, privacy, sample-start, linkedin, second-factor, second-factor-disabled, totp, totp-disabled, totp-prod-flag-off, totp-prod-enabled, totp-prod-cleanup, or mcp-sdk' <<<"$output" ||
