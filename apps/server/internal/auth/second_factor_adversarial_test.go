...
func newSFEnv(t *testing.T, service *sfService) *sfEnv {
	...
	issued, err := auth.NewPendingAuthenticationManager(pool, nil).Create(context.Background(), auth.PendingAuthenticationRequest{
...
func (e *sfEnv) session(t *testing.T) (string, store.Session) {
	t.Helper()
	raw, sess, err := e.sessions.Issue(context.Background(), e.userID, "test-agent", "203.0.113.7")
	if err != nil {
...
func TestSecondFactorPending_OutcomesAndCookies(t *testing.T) {
	...
	rec := sfServe(e.mux, e.verify(sfOrigin, e.pendingCSRF()))
...
func TestSecondFactorPending_RateAdmissionFollowsCSRF(t *testing.T) {
	...
	e := newSFEnv(t, service)
	for i := 0; i < 10; i++ {
		if rec := sfServe(e.mux, e.verify(sfOrigin, e.pendingCSRF())); rec.Code != http.StatusBadRequest {
...
func TestSecondFactorPendingStatus_ReadsOnlyThePendingRow(t *testing.T) {
	...
	e := newSFEnv(t, service)
	rec := sfServe(e.mux, sfRequest{method: http.MethodGet, path: auth.SecondFactorPendingPath, cookies: []*http.Cookie{e.pendingCookie()}})
	var body struct {
		Data struct {
	...
	service.methods = nil
	if rec = sfServe(e.mux, sfRequest{method: http.MethodGet, path: auth.SecondFactorPendingPath, cookies: []*http.Cookie{e.pendingCookie()}}); rec.Code != http.StatusServiceUnavailable {
		t.Fatalf("status with no available method = %d, want 503", rec.Code)
	}
	raw, _ := e.session(t)
	if rec = sfServe(e.mux, sfRequest{method: http.MethodGet, path: auth.SecondFactorPendingPath, cookies: []*http.Cookie{{Name: "__Host-session", Value: raw}}}); rec.Code != http.StatusUnauthorized {
...
func TestSecondFactorPendingReauth_RequiresTheBoundSession(t *testing.T) {
	...
	reauth, err := auth.NewPendingAuthenticationManager(e.pool, nil).Create(context.Background(), auth.PendingAuthenticationRequest{
	...
	for name, cookies := range map[string][]*http.Cookie{
		...
		if rec := sfServe(e.mux, sfRequest{method: http.MethodGet, path: auth.SecondFactorPendingPath, cookies: cookies}); rec.Code != http.StatusUnauthorized {
			t.Errorf("reauth status with %s = %d, want 401", name, rec.Code)
		}
	}
	rec := sfServe(e.mux, sfRequest{method: http.MethodGet, path: auth.SecondFactorPendingPath, cookies: []*http.Cookie{pendingCookie, {Name: "__Host-session", Value: raw}}})
...
func TestSecondFactorAccountMutations_SessionCSRFAndResponses(t *testing.T) {
	...
	for name, req := range map[string]sfRequest{
		...
		if rec := sfServe(e.mux, req); rec.Code != http.StatusForbidden {
	...
	if rec := sfServe(e.mux, sfRequest{method: http.MethodDelete, path: auth.SecondFactorPasskeysPath + "/not-a-uuid", origin: sfOrigin, csrf: csrf, cookies: sessionCookie}); rec.Code != http.StatusNotFound || sfErrorCode(t, rec) != "factor_not_found" {
		t.Fatalf("malformed passkey ID = %d %s", rec.Code, rec.Body)
	}
	if rec := sfServe(e.mux, remove); rec.Code != http.StatusNoContent || !sfSetsCookie(rec, "__Host-session", "replacement") {
	...
	for _, tc := range []struct {
		...
		service.err = tc.err
		if rec := sfServe(e.mux, remove); rec.Code != tc.code || sfErrorCode(t, rec) != tc.errCode {
	...
	if rec := sfServe(e.mux, regenerate); rec.Code != http.StatusOK || rec.Body.String() != `{"data":{"recoveryCodes":["amr_new"]}}`+"\n" {
	...
	rec = sfServe(e.mux, sfRequest{method: http.MethodGet, path: auth.SecondFactorStatePath, cookies: sessionCookie})
...
func TestSecondFactorRegistrationComplete_DisabledConsumesThenNotFound(t *testing.T) {
	...
	req.csrf = ""
	if rec = sfServe(e.mux, req); rec.Code != http.StatusNotFound || service.completeCalls != 1 {
