...
func TestResolveLinkedInUser_LinkPurpose_MissingLinkingUserID_RejectsGenerically(t *testing.T) {
	...
	// Issuer/endpoint overrides are never dialed by this test
	// (resolveLinkedInUser does no OIDC discovery or GitHub API call of
	// its own), so every override below is left empty -- NewServiceForTest
	// itself substitutes its own unroutable sentinel for each empty one
	// (see that function's doc comment), so there is no need to inline a
	// placeholder value here.
	svc, err := auth.NewServiceForTest(testLogger(), cfg, q, "", "", "")
	if err != nil {
