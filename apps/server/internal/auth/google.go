...
// resolveLoginIdentity's (handlers.go, Task 10) email-local-part fallback
// is the exception, not the default, for a real Google login -- Google
// still doesn't guarantee the claim is present even with this scope
// granted, and oidctest's Claims has no Name field at all, so every test
...
// this package requires); see resolveLoginIdentity (handlers.go) for the
