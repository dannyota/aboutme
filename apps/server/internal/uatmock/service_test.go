...
func TestAuthorizationGETRendersAccessibleAccountForm(t *testing.T) {
	...
	for _, want := range []string{"<form", `method="post"`, `action="` + authorizePath + `"`, "Development User", "developer@example.invalid", "Alice Local", "alice@example.invalid", "Bob Local", "bob@example.invalid", "Provider Only", "pa-provider-only@example.invalid", "Password Link", "pa-link@example.invalid", "MCP Proof", "mcp-proof@example.invalid", "Continue with Google"} {
