...
func TestRevisionSerializedAsString(t *testing.T) {
	...
	get := resumeRequest(t, h, http.MethodGet, apiResumePath+"/"+created.ID.String(), "", 0, uuid.Nil, "4")
	...
	list := resumeRequest(t, h, http.MethodGet, apiResumePath, "", 0, uuid.Nil, "4")
	...
	stale := resumeRequest(t, h, http.MethodPatch, apiResumePath+"/"+created.ID.String(), `{"title":"stale"}`, revision-1, uuid.New(), "4")
	...
	written := resumeRequest(t, h, http.MethodPatch, apiResumePath+"/"+created.ID.String(), `{"title":"exact"}`, revision, uuid.New(), "4")
...
func TestByteVsCodePointBounds(t *testing.T) {
	...
	t.Run("rich text counts UTF-8 bytes", func(t *testing.T) {
		...
		response := resumeRequest(t, h, http.MethodPatch, path, body, created.Revision, uuid.New(), "4")
	...
	t.Run("title counts code points", func(t *testing.T) {
		...
		accepted := resumeRequest(t, h, http.MethodPatch, path,
			string(mustResumeTestJSON(t, map[string]any{"title": atLimit})), created.Revision, uuid.New(), "4")
		...
		rejected := resumeRequest(t, h, http.MethodPatch, path,
			string(mustResumeTestJSON(t, map[string]any{"title": atLimit + "😀"})), created.Revision+1, uuid.New(), "4")
...
func testBoundsHTTPRequestBytes(t *testing.T, contract boundsHTTPContract) {
	...
	accepted := resumeRequest(t, h, http.MethodPost, apiResumePath, body, 0, uuid.New(), "4")
	...
	before := snapshotBoundsHTTPWriteState(t, h)
	rejected := resumeRequest(t, h, http.MethodPost, apiResumePath, body+" ", 0, uuid.New(), "4")
...
func testBoundsHTTPDocumentBytes(t *testing.T, contract boundsHTTPContract) {
	...
	path := apiResumePath + "/" + created.ID.String()
	accepted := resumeRequest(t, h, http.MethodPatch, path,
		`{"title":"aggregate bound accepted"}`, created.Revision, uuid.New(), "4")
	if accepted.status != http.StatusOK {
	...
	before := snapshotBoundsHTTPWriteState(t, h)
	rejected := resumeRequest(t, h, http.MethodPatch, path+"/entries/work", body, created.Revision+1, uuid.New(), "4")
	assertRouteError(t, rejected, http.StatusUnprocessableEntity, "document_invalid")
...
func testBoundsHTTPSections(t *testing.T, contract boundsHTTPContract) {
	...
	accepted := resumeRequest(t, h, http.MethodPatch, path, atLimitBody, created.Revision, uuid.New(), "4")
	if accepted.status != http.StatusOK {
	...
	rejected := resumeRequest(t, h, http.MethodPatch, path, overBody, created.Revision+1, uuid.New(), "4")
	assertRouteError(t, rejected, http.StatusUnprocessableEntity, "document_invalid")
...
func testBoundsHTTPEntries(t *testing.T, contract boundsHTTPContract) {
	...
	accepted := resumeRequest(t, h, http.MethodPatch, path,
		`{"entry":{"id":"20000000-0000-4000-8000-000000000064"}}`, created.Revision, uuid.New(), "4")
	if accepted.status != http.StatusOK {
		t.Fatalf("entry %d = %d %s, want 200", contract.entriesPerSection, accepted.status, accepted.body)
	}
	before := snapshotBoundsHTTPWriteState(t, h)
	rejected := resumeRequest(t, h, http.MethodPatch, path,
		`{"entry":{"id":"20000000-0000-4000-8000-000000000065"}}`, created.Revision+1, uuid.New(), "4")
	assertRouteError(t, rejected, http.StatusUnprocessableEntity, "document_invalid")
...
func testBoundsHTTPRichText(t *testing.T, contract boundsHTTPContract) {
	...
	accepted := resumeRequest(t, h, http.MethodPatch, path, richTextEntryBody(t, atLimit), created.Revision, uuid.New(), "4")
	if accepted.status != http.StatusOK {
		t.Fatalf("rich text at %d bytes = %d %s, want 200", contract.richTextBytes, accepted.status, accepted.body)
	}
	before := snapshotBoundsHTTPWriteState(t, h)
	rejected := resumeRequest(t, h, http.MethodPatch, path, richTextEntryBody(t, atLimit+"a"), created.Revision+1, uuid.New(), "4")
	assertRouteError(t, rejected, http.StatusUnprocessableEntity, "document_invalid")
...
func testBoundsHTTPPersonalDetails(t *testing.T, contract boundsHTTPContract) {
	...
	accepted := resumeRequest(t, h, http.MethodPatch, path, bodyFor(contract.personalDetails), created.Revision, uuid.New(), "4")
	if accepted.status != http.StatusOK {
		t.Fatalf("%d details = %d %s, want 200", contract.personalDetails, accepted.status, accepted.body)
	}
	before := snapshotBoundsHTTPWriteState(t, h)
	rejected := resumeRequest(t, h, http.MethodPatch, path, bodyFor(contract.personalDetails+1), created.Revision+1, uuid.New(), "4")
	assertRouteError(t, rejected, http.StatusUnprocessableEntity, "document_invalid")
...
func testBoundsHTTPLanguage(t *testing.T, contract boundsHTTPContract) {
	...
	path := apiResumePath + "/" + created.ID.String()
	accepted := resumeRequest(t, h, http.MethodPatch, path,
		string(mustResumeTestJSON(t, map[string]any{"lng": atLimit})), created.Revision, uuid.New(), "4")
	if accepted.status != http.StatusOK {
		t.Fatalf("lng at %d code points = %d %s, want 200", contract.lngCodePoints, accepted.status, accepted.body)
	}
	before := snapshotBoundsHTTPWriteState(t, h)
	rejected := resumeRequest(t, h, http.MethodPatch, path,
		string(mustResumeTestJSON(t, map[string]any{"lng": limitPlusOne})), created.Revision+1, uuid.New(), "4")
	assertRouteError(t, rejected, http.StatusUnprocessableEntity, "document_invalid")
...
func TestNoUnboundedWork(t *testing.T) {
	...
	for _, request := range requests {
		t.Run(request.name, func(t *testing.T) {
			response := resumeRequest(t, h, request.method, request.path, request.body, request.revision, uuid.New(), "4")
			assertRouteError(t, response, http.StatusUnprocessableEntity, "document_invalid")
