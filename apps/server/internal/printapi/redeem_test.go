...
func TestQueueFailuresReplayAndPayloadBindingAreOpaque(t *testing.T) {
	...
	mutations := []struct {
		...
		{"duplicate nested key", func(s *renderjob.Snapshot) {
			s.Payload = bytes.Replace(s.Payload, []byte(`"schemaVersion":4`), []byte(`"schemaVersion":4,"schemaVersion":4`), 1)
		}},
		...
		{"payload schema mismatch", func(s *renderjob.Snapshot) {
			s.Payload = bytes.Replace(s.Payload, []byte(`"schemaVersion":4`), []byte(`"schemaVersion":5`), 1)
		}},
