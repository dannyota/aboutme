...
// version is 5) and returns it. The row lands at production schema_version 4,
...
func TestStore_Integration_BackfillOne_OldVersionRow_Applied(t *testing.T) {
	...
	if before.SchemaVersion != 4 {
		t.Fatalf("seeded schema_version = %d, want 4 (the row must start BELOW the projector's current)", before.SchemaVersion)
	}
	...
	if after.SchemaVersion != 5 {
		t.Errorf("schema_version after backfill = %d, want 5", after.SchemaVersion)
	}
...
func TestStore_Integration_BackfillOne_ConcurrentAutosaveInGap_LostRace(t *testing.T) {
	...
	if after.SchemaVersion != 4 {
		t.Errorf("schema_version = %d, want 4 (the backfill must have written nothing)", after.SchemaVersion)
	}
	...
	if v := bfSchemaVersion(ctx, t, pool, created.ID); v != 5 {
		t.Errorf("schema_version after the retry = %d, want 5", v)
	}
...
func TestStore_Integration_BackfillOne_TitleOnlyWriteInGap_LostRace(t *testing.T) {
	...
	if after.SchemaVersion != 4 {
		t.Errorf("schema_version = %d, want 4: a title write never migrates a document, so this lost race is a retry signal, not completion", after.SchemaVersion)
	}
