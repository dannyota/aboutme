...
func pjDerivedNextSchema(t *testing.T) []byte {
	t.Helper()
	dec := json.NewDecoder(bytes.NewReader(pjReleasedRawSchema(t)))
	dec.UseNumber()
...
func pjHeadlineConverter(t *testing.T, target int32, add bool) docmigrate.ConvertFunc {
	...
	return func(doc json.RawMessage) (json.RawMessage, error) {
		...
		if h, ok := pd["headline"].(string); ok {
			if add {
				pd["headline"] = pjNextPrefix + h
			} else {
				if !strings.HasPrefix(h, pjNextPrefix) {
					return nil, fmt.Errorf("headline %q does not carry the %q prefix", h, pjNextPrefix)
				}
				pd["headline"] = strings.TrimPrefix(h, pjNextPrefix)
			}
		}
...
func TestStore_Integration_Get_ProjectsOldVersionRow_WithoutWriting(t *testing.T) {
	...
	for i := range readers {
		...
		go func(i int) {
			...
			if r.Doc.PersonalDetails.Headline == nil || *r.Doc.PersonalDetails.Headline != pjNextPrefix+stored {
	...
	if want := pjNextPrefix + stored; pjHeadline(t, list[0].Doc) != want {
