...
// shapes match the domain layer. A failure here means migrations, queries.sql,
// or the sqlc.yaml overrides drifted from the expected shape. The field-level
// lines pin the nullable-column contract and the sqlc.yaml `rename` entry that
// keeps seo_geo_enabled from surfacing as
// SeoGeoEnabled -- which breaks the Go initialism rule and would disagree,
// by casing alone, with the domain field SEOGeoEnabled.
var (
...
// json.RawMessage -- which would not catch a dropped or altered jsonb/bytea
// override in sqlc.yaml that changes the generated field's type. A pointer
// closes the
