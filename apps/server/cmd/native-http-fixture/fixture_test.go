...
func TestParseConfigAcceptsSeed(t *testing.T) {
	...
	cmd, cfg, err := parseConfig([]string{
		...
		"--media-root", ".dev/native-http-fixture-media",
	...
	if want := filepath.Join(root, ".dev/native-http-fixture-media"); cfg.MediaRoot != want {
...
func TestParseConfigDatabaseURLValidation(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			args := []string{"seed", "--database-url", tc.dsn, "--media-root", ".dev/native-http-fixture-media", "--now", "2035-01-01T00:00:00Z"}
...
func TestParseConfigRejectsMissingDatabaseURL(t *testing.T) {
	_, _, err := parseConfig([]string{"seed", "--media-root", ".dev/native-http-fixture-media", "--now", "2035-01-01T00:00:00Z"}, t.TempDir())
	if err == nil {
...
func TestParseConfigMediaRootValidation(t *testing.T) {
	...
	cases := []struct {
		...
		{"outside repository", "/tmp/native-http-fixture-media"},
		{"dotdot escape", filepath.Join(root, "..", "native-http-fixture-media")},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
...
func TestParseConfigNowValidation(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			args := []string{"seed", "--database-url", frozenDSN, "--media-root", ".dev/native-http-fixture-media", "--now", tc.now}
			_, _, err := parseConfig(args, t.TempDir())
			if err == nil {
...
func TestParseConfigRejectsMissingNow(t *testing.T) {
	_, _, err := parseConfig([]string{"seed", "--database-url", frozenDSN, "--media-root", ".dev/native-http-fixture-media"}, t.TempDir())
	if err == nil {
...
func TestBuildSeedPlanDeterministicValues(t *testing.T) {
	cfg := Config{
		...
		MediaRoot:   filepath.Join(t.TempDir(), ".dev", "native-http-fixture-media"),
