...
web-e2e-update: ## Generate review-only renderer baseline candidates in the pinned AMD64 browser
	...
	podman run --rm --platform linux/amd64 --network=host --security-opt label=disable \
	  ...
	  -e TZ=UTC -e LANG=C.UTF-8 -e LC_ALL=C.UTF-8 \
	  -e PLAYWRIGHT_RESULTS_DIR=/results -w /tmp '$(WEB_E2E_IMAGE)' \
	  sh -eu -c 'test "$$(uname -m)" = x86_64; test "$$TZ" = UTC; test "$$LANG" = C.UTF-8; test "$$LC_ALL" = C.UTF-8; locale -a | grep -qx C.utf8; test "$$(locale charmap)" = UTF-8; chrome_version="$$(/ms-playwright/chromium-1234/chrome-linux64/chrome --version)"; chrome_version="$$(printf "%s" "$$chrome_version" | sed "s/[[:space:]]*$$//")"; test "$$chrome_version" = "Google Chrome for Testing 151.0.7922.34"; mkdir /tmp/aboutme; tar -xf /candidate.tar -C /tmp/aboutme; test ! -e /tmp/aboutme/.git; test ! -e /tmp/aboutme/.env; test ! -e /tmp/aboutme/.dev; test ! -e /tmp/aboutme/.superpowers; cd /tmp/aboutme/apps/web; npm ci --ignore-scripts; status=0; PLAYWRIGHT_SURFACE=harness npx --no-install playwright test --config e2e/playwright.config.ts screenshot.spec.ts fonts-offline.spec.ts corpus.spec.ts print.spec.ts samples.spec.ts sample-pages.spec.ts preview-gap.spec.ts --update-snapshots || status=$$?; if [ "$$status" -eq 0 ]; then PLAYWRIGHT_SURFACE=normal npx --no-install playwright test --config e2e/playwright.config.ts normal-csp.spec.ts gallery.spec.ts chrome.spec.ts --update-snapshots || status=$$?; fi; post=0; for path in playwright-report test-results blob-report; do if [ -e "/tmp/aboutme/apps/web/$$path" ]; then echo "unexpected default Playwright output: $$path" >&2; post=1; fi; done; if [ "$$status" -eq 0 ]; then expected=/tmp/expected-baselines; actual=/tmp/actual-baselines; printf "%s\n" baselines/classic-serif--vn-full--paged.png baselines/classic-serif--vn-full--justify--paged.png baselines/engineer-compact--vn-full--paged.png baselines/modern-sidebar--vn-full--paged.png baselines/executive-band--vn-full--paged.png baselines/consulting-formal--vn-full--paged.png baselines/consulting-formal--vn-full--letter--paged.png baselines/academic-dense--vn-full--paged.png baselines/modern-sidebar--full--continuous.png baselines/public--classic-serif--2560.png baselines/public--modern-sidebar--2560.png baselines/public--modern-sidebar--390.png baselines/chrome--home--light--390.png baselines/chrome--home--light--1440.png baselines/chrome--home--dark--390.png baselines/chrome--home--dark--1440.png baselines/chrome--login--light--390.png baselines/chrome--login--light--1440.png baselines/chrome--login--dark--390.png baselines/chrome--login--dark--1440.png print-baselines/print-main-overflow-p1.png print-baselines/print-main-overflow-p2.png print-baselines/print-sidebar-overflow-p1.png print-baselines/print-sidebar-overflow-p2.png template-pages/manifest.json template-pages/ats-plain-en-p1.png template-pages/ats-plain-vi-p1.png template-pages/engineer-compact-en-p1.png template-pages/engineer-compact-vi-p1.png template-pages/executive-band-en-p1.png template-pages/executive-band-en-p2.png template-pages/executive-band-vi-p1.png template-pages/executive-band-vi-p2.png template-pages/graduate-friendly-en-p1.png template-pages/graduate-friendly-vi-p1.png template-pages/modern-sidebar-en-p1.png template-pages/modern-sidebar-vi-p1.png | LC_ALL=C sort >"$$expected"; test -d /results/candidate-baselines || { echo "candidate baseline directory is missing" >&2; post=1; }; if find /results/candidate-baselines -type l -print -quit | grep -q .; then echo "candidate baseline contains a symlink" >&2; post=1; fi; if find /results/candidate-baselines ! -type d ! -type f -print -quit | grep -q .; then echo "candidate baseline contains a special file" >&2; post=1; fi; find /results/candidate-baselines -type f -printf "%P\n" | LC_ALL=C sort >"$$actual"; diff -u "$$expected" "$$actual" || post=1; if [ "$$post" -eq 0 ]; then (cd /results/candidate-baselines && sha256sum $$(cat "$$expected")) > /results/candidate-baselines/SHA256SUMS; fi; fi; test "$$post" -eq 0 || exit 1; exit "$$status"'; \
	test "$$(git rev-parse --verify 'HEAD^{commit}')" = "$$commit"; \
	test -z "$$(git status --porcelain=v1 --untracked-files=all)"
