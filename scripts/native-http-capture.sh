#!/usr/bin/env bash
# Deterministic public HTTP capture. It seeds the frozen fixture into an
...
readonly FIXTURE_RUNTIME=.dev/native-http-fixture-runtime
readonly FIXTURE_MEDIA=.dev/native-http-fixture-media
readonly FIXTURE_EVIDENCE=.dev/native-http-evidence
...
cleanup() {
  ...
  if [ -x "$ROOT/.dev/bin/native-http-fixture" ]; then
    native-http-fixture cleanup --database-url "$FIXTURE_DATABASE_URL" --media-root "$FIXTURE_MEDIA" --now "$FROZEN_NOW" >/dev/null 2>&1
  fi
...
(cd "$ROOT/apps/server" && go build -o "$ROOT/.dev/bin/native-http-fixture" ./cmd/native-http-fixture)

info "--- seeding fixture"
native-http-fixture seed --database-url "$FIXTURE_DATABASE_URL" --media-root "$FIXTURE_MEDIA" --now "$FROZEN_NOW"
