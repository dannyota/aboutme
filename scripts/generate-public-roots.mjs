...
const registryRelativePath = "packages/publicroots/public-roots.v9.json";
...
export function parsePublicRoots(raw) {
  ...
  if (!Number.isInteger(value.version) || value.version !== 9) {
    throw new Error("public roots registry version must be integer 9");
  }
  if (
  ...
  value.roots.forEach((row, index) => {
    ...
    if (row.root !== expected.root || row.dispatch !== expected.dispatch) {
      throw new Error(
        `public roots row ${index} does not match the v9 authority`,
      );
    }
  });
  return value;
