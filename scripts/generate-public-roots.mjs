...
const registryRelativePath = "packages/publicroots/public-roots.v4.json";
...
export function parsePublicRoots(raw) {
  ...
  if (!Number.isInteger(value.version) || value.version !== 4) {
    throw new Error("public roots registry version must be integer 4");
  }
  if (
  ...
  value.roots.forEach((row, index) => {
    ...
    if (row.root !== expected.root || row.dispatch !== expected.dispatch) {
      throw new Error(
        `public roots row ${index} does not match the v4 authority`,
      );
    }
  });
  return value;
