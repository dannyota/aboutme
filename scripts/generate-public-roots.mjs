...
const registryRelativePath = "packages/publicroots/public-roots.v7.json";
...
export function parsePublicRoots(raw) {
  ...
  if (!Number.isInteger(value.version) || value.version !== 7) {
    throw new Error("public roots registry version must be integer 7");
  }
  if (
  ...
  value.roots.forEach((row, index) => {
    ...
    if (row.root !== expected.root || row.dispatch !== expected.dispatch) {
      throw new Error(
        `public roots row ${index} does not match the v7 authority`,
      );
    }
  });
  return value;
