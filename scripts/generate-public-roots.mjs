...
const registryRelativePath = "packages/publicroots/public-roots.v8.json";
...
export function parsePublicRoots(raw) {
  ...
  if (!Number.isInteger(value.version) || value.version !== 8) {
    throw new Error("public roots registry version must be integer 8");
  }
  if (
  ...
  value.roots.forEach((row, index) => {
    ...
    if (row.root !== expected.root || row.dispatch !== expected.dispatch) {
      throw new Error(
        `public roots row ${index} does not match the v8 authority`,
      );
    }
  });
  return value;
