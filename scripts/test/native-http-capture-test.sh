...
# Static contract for the native public HTTP capture. It proves the
...
fail() {
  printf 'native-http-capture-test: %s\n' "$*" >&2
  exit 1
}
...
test_fixture() {
  local bin
  fixture_tmp=$(mktemp -d "$ROOT/native-http-capture-test.XXXXXX")
  bin=$fixture_tmp/native-http-fixture
  (cd "$ROOT/apps/server" && go build -o "$bin" ./cmd/native-http-fixture) ||
    fail 'native-http-fixture does not build'
  ...
  set +e
  closed_out=$("$bin" seed --database-url "$closed_dsn" --media-root .dev/native-http-fixture-media --now "$FROZEN_NOW" 2>&1)
  closed_status=$?
  set -e
  ...
  out=$(rejects 'non-loopback database' seed --database-url 'postgres://aboutme:aboutme_dev@10.90.0.2:20432/aboutme_p5a_fixture' --media-root .dev/native-http-fixture-media --now "$FROZEN_NOW")
  [[ $out == *loopback* ]] || fail "non-loopback database error should mention loopback: $out"

  out=$(rejects 'non-fixture database' seed --database-url 'postgres://aboutme:aboutme_dev@127.0.0.1:20432/aboutme_dev?sslmode=disable' --media-root .dev/native-http-fixture-media --now "$FROZEN_NOW")
  ...
  out=$(rejects 'escaping media root' seed --database-url "$FROZEN_DSN" --media-root /tmp/native-http-outside --now "$FROZEN_NOW")
  [[ $out == *'below the repository root'* ]] || fail "escaping media error: $out"

  out=$(rejects 'non-frozen clock' seed --database-url "$FROZEN_DSN" --media-root .dev/native-http-fixture-media --now '2020-01-01T00:00:00Z')
  ...
  printf '%s\n' 'native-http-capture-test: fixture PASS'
}
...
test_lifecycle() {
  local capture=$ROOT/scripts/native-http-capture.sh
  [ -f "$capture" ] || fail 'scripts/native-http-capture.sh is missing'
  [ -x "$capture" ] || fail 'scripts/native-http-capture.sh is not executable'
  ...
  [[ $content == *'.dev/native-http-fixture-runtime'* ]] ||
    fail 'capture script must use the rooted .dev/native-http-fixture-runtime state dir'
  [[ $content == *'.dev/native-http-fixture-media'* ]] ||
    fail 'capture script must use the rooted .dev/native-http-fixture-media media dir'
  [[ $content == *'trap'* ]] ||
    fail 'capture script must install a cleanup trap'
  [[ $content == *'native-http-fixture seed'* ]] ||
    fail 'capture script must seed via native-http-fixture'
  [[ $content == *'native-http-fixture cleanup'* ]] ||
    fail 'capture script must clean via native-http-fixture'
  [[ $content == *'ABOUTME_DEV_DATABASE_URL'* ]] ||
    fail 'capture script must isolate the database via ABOUTME_DEV_DATABASE_URL'

  grep -q '^native-http-check:' <<<"$makefile" ||
    fail 'Makefile is missing the native-http-check target'

  for f in resume-v2.json photo.png hashes.json; do
    [ -f "$ROOT/apps/server/cmd/native-http-fixture/testdata/$f" ] ||
