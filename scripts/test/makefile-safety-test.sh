...
cp "$ROOT/deploy/dev-https-browser/"{Dockerfile,package.json,package-lock.json,playwright.config.ts,auth.spec.ts,transport.spec.ts,editor.spec.ts,public.spec.ts,password-auth.spec.ts,mcp.spec.ts,mcp-sdk.spec.ts,second-factor.spec.ts,entry.spec.ts,publish.spec.ts,exports.spec.ts,privacy.spec.ts,sample-start.spec.ts,editor-fixtures.ts,network-policy.ts,harness-lib.ts,run.sh,verify-evidence.mjs} \
