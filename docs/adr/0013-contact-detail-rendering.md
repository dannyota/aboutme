...
**(c) `label`.** `personalDetail.label` **ships as part of this decision**. When
present and non-empty it replaces the type's default label; when absent or `""`
the type's default label is used. This is the absent-versus-cleared distinction
the document contract already guarantees everywhere else, and it costs one
...
- The first goldens pin plain-text contacts and label substitution. Reversing
  ...
  is why they are ruled now rather than discovered later.
