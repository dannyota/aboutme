...
Status: Proposed (2026-09-26). Supersedes decision 2 of
...
or rejects the remaining risk below.
...
steals a victim's unused authorization code starts their own sign-in and
presents the victim's code with their own state and transaction cookie. The
callback redeems it and signs the attacker in as the victim, or, in a link flow,
attaches the victim's LinkedIn identity to the attacker's account.
...
- The code is single use and lives at most 30 minutes [2]. The victim's own
...
before delivery, for example by a malicious browser extension. Such an attacker
can usually take the resulting session cookie as well.
