...
Design spec §2 states the rule directly: "Editor pagination is **approximate**;
the PDF (CSS `@page`) is **authoritative** — JS measurement and print engine are
...
- The editor's page count is **advisory** and must be labelled as an estimate in
  ...
  depend on it.
- The server never accepts a client-supplied page count. FlowCV's download
  endpoint takes a `previewPageCount` query parameter
  (`/home/danny/src/flowcv/docs/RENDERING.md`); we deliberately do not, because
  it makes a client measurement load-bearing in a server render.

That document is also the reference for the two-renderer split itself — a
client-side HTML preview and a separate server-side PDF built from the same
resume JSON — which is the arrangement design spec §2 already specifies for us.
...
- Margins default to **15 mm on all four sides for both formats**. Since
  2026-08-11 `spacing.pageMargin` (`{x, y}` in mm, 0–40 per axis) overrides
  them; absent, `useResumeStyles` applies the 15 mm fallback at the point of use
  and never writes it back. Consumer printers cannot print closer than about 5
  mm to the paper edge, so margins below that produce clipped hardware output
  even though the PDF looks correct.
...
| `.section-lead` (heading + first entry header)     | keeps a heading attached to something, not only to whitespace  | `break-inside: avoid`                     |
...
`.section-lead` is a wrapper element the renderer emits around the heading and
the first entry's header. It exists only to make "heading plus something"
unbreakable; without it, `break-after: avoid` on the heading can still leave the
heading and a single meta line together at a page foot.
...
the rules above, the golden PDFs record what it actually does and the divergence
is raised to the design owner — the fix is never a per-template workaround.

## 6. Photo and images

The print browser has **no general outbound network access** (design spec §5).
Everything the page needs must arrive with it.
...
Golden HTML snapshots and Playwright screenshot diffs (design spec §5) are only
meaningful if the same document renders to the same bytes. Every input that can
vary is pinned.

| Input              | Pin                                                                                                                                       |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------- |
| Chromium           | image digest, per §0's pinned-versions rule; a browser upgrade is a deliberate snapshot-refresh change                                    |
| Fonts              | self-hosted subsets, no external CDN; weights restricted to 400 and 700 so nothing is synthesized (`tokens.md` §3.2)                      |
| Font loading       | `await document.fonts.ready` before print                                                                                                 |
| Timezone           | `TZ=UTC` in the render container                                                                                                          |
| Locale             | explicit `LANG`/`LC_ALL`, and an explicit `lang` on the resume root from `resumes.lng` — `text-transform: capitalize` is locale-sensitive |
| Clock              | the renderer makes no `Date.now()` or locale calls (§5); `present: true` renders a fixed label, never a computed "now"                    |
...
| Contrast clamp     | pure function, fixed step size (`tokens.md` §5); its outputs are pinned by the snapshots                                                  |
...
Operationally, prints run one at a time inside the Go task's 512 MB whole-task
budget, with a 20 s timeout and process-group kill (design spec §2). A template
that needs more than that budget to render is a template defect, not a capacity
problem: the guard is that every golden fixture prints well inside the timeout.
