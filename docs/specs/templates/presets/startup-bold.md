...
1.00x title, both 700. _Family:_ Inter's x-height is the highest of the five.

**2. No rule, because the weight is the boundary.** `showRule: false`.
`tokens.md` §5 calls the rule decorative because "the heading's size and weight
...
`rule` are the `tokens.md` §4 derivations read as sRGB-channel mixes.
...
- Applying this resets `pageFormat` to `letter` (`contract.md` §9.2): an A4 user
  ships a Letter PDF unwarned; containment is editor-side.
