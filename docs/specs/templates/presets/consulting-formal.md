...
| `columns: 1` with `placement: "keep"`               | Applying moves nothing, and a document already using a sidebar renders it below `main` (§7). `sidebarSectionTypes` is `[]` — inert under `keep`, and stated deliberately.                                                              |
...
preset ships a Letter PDF with no signal from the preset layer (§9.2) — 5.9 mm
wider and 17.6 mm shorter here, so the page re-paginates, trading ~3.5 lines of
height for one point of measure. `pageFormat` has no `"keep"`, so containment is
the editor's job; picking `a4` would only hide the cost behind a template that
prints wrong for its readers. The 25 mm side margins absorb a flip back to `a4`,
which still gives a 160 mm measure.
...
- **A header rule, or an icon-free page.** `showRule` covers section headings
  only, and `iconKey` still renders despite `iconStyle: "none"` (§9.5).
