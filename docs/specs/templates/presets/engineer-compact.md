...
1. **An untinted sidebar** (`surfaceTarget: "none"`, no `colors.surface`). The
   ...
   first.
2. **Base 13 px, no smaller** — 9.75 pt, the floor `tokens.md` §5 names before
...
derived roles mixed in sRGB per `tokens.md` §4. An OKLab mix changes no verdict.
...
- **No sidebar width.** `--sidebar-ratio` is fixed at 32% (`contract.md` §9.6),
  so `spacing.pageMargin.x` is the only lever on it — hence 11 mm, not 15 mm.
