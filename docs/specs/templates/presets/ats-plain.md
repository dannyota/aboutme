...
| Header details  | `stacked`, `left`                 | One line box per detail, so every position-based extractor breaks on the baseline change. Inline details are separated by a `0.5em` CSS gap (`tokens.md` §6), not a glyph, and gap-to-space thresholds are per-implementation. Left keeps one constant left edge for x-clustering.                                                                                                                    | OBSERVED (stacked), INFERRED (align)               |
...
| Colour          | `#000000` on `#ffffff`, no accent | 21:1 is a fixed point of the contrast clamp (`tokens.md` §5): what the preset specifies is exactly what renders, in preview, SSR and print. Maximum ink-to-paper separation is also the most robust input to OCR binarisation.                                                                                                                                                                        | OBSERVED (clamp), INFERRED (OCR)                   |
...
| Type            | Source Sans 3, 14 px              | No parse argument separates the five families: Chromium embeds a ToUnicode CMap per subset, and Phase 3 Task 5 pins complete Vietnamese precomposed coverage (U+1EA0–1EF9) for all five behind a `fontkit` cmap test, so extraction is family-independent. Chosen instead for the OCR fallback — a crossbarred capital `I`, distinct from `l` — and 14 px = 10.5 pt.                                  | OBSERVED (neutrality), INFERRED (letterforms)      |
...
| -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| "A non-standard font breaks ATS"             | False in this pipeline. Chromium writes a ToUnicode CMap for every embedded subset, so extraction returns the stored characters whichever of the five families is set. The font choice above is legibility, not parseability. |
...
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Photo cannot be suppressed (`contract.md` §9.3)  | `fixtures/full.json` carries one, so this preset's golden print shows a 96 px photo. Per the table above that is not an extraction hazard, but it is not this preset's intent either, and the only user workaround loses the crop.                                                                                       |
| `iconKey` has no global off switch (§9.5)        | `header.iconStyle` covers header details only. All eight sections of `full.json` carry an `iconKey`, so eight lucide SVGs render. Heading text still extracts unchanged; the cost is x-alignment in layout analysis, plus the OCR path.                                                                                  |
| Range separator is U+2013 (§5.4), renderer-fixed | An ASCII hyphen or the word "to" would be safer for tokenisers that split on `-`. Worse, U+2013 is outside every subset range pinned by Phase 3 Task 5 (Basic Latin, Latin-1 letters, Vietnamese, U+2018–201D, U+2026), so it falls back to a container font — a determinism risk for every template, not only this one. |
...
| Apply resets `pageFormat` (§9.2)                 | A US applicant who picks this preset silently ships A4.                                                                                                                                                                                                                                                                  |
