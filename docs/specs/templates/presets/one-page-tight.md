...
**8.78 pt**. The 13 px floor is `tokens.md` §5's, not mine; 11 px would have
...
- **The fit is a claim, not enforced.** Nothing measures the page, the editor
  count is advisory, and a photo overruns the header (`contract.md` §9.3).
- **`--sidebar-ratio` is fixed at 32 %.** Main binds in every fit above, so a
  template-level ratio would beat the margin token (`tokens.md` §6).
