...
`packages/schema/templates/nordic-muted.json`.
...
Per `tokens.md` §4; mixes are sRGB lerps; values are pre-clamp. All pass.
...
- `spacing.pageMargin` contradicts `print.md` §2's fixed 15 mm; this preset
  assumes the token wins once it lands.
