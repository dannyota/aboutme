...
| D13 | Whether media needs its own table and how crashes are repaired                                                                                                         | **No media-table or media-key migration.** The only P2B migration is the unrelated bounded-idempotency-retention change from Task 2. The document owns the current key. Upload writes a new immutable object, then commits the document. Failure best-effort deletes the new object. Replace or delete removes the reference before best-effort cleanup of the exact D11-validated old key; an invalid key or cleanup failure leaves unreachable bytes. The bounded 48-hour orphan sweep in `../budgets.md` repairs those objects. Rejected: a media table, which adds a second write model without making object storage transactional                                                                                                                                                                                                   |
...
request or file bytes return `413 media_too_large`. Every rejection leaves
PostgreSQL and object storage unchanged.
