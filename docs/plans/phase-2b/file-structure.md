...
| `apps/server/internal/media/{media.go,media_test.go,fs.go,s3.go,conformance_test.go,fs_test.go,s3_test.go}`                                             | Backend interface, filesystem + S3-compatible backends, exact resume-photo key constructor/parser, one shared contract (T3)        |
| `apps/server/internal/media/{admission.go,admission_test.go,normalize.go,normalize_test.go}`                                                            | Task-wide permit, bounded decode, and normalization (T11)                                                                          |
...
| `apps/server/internal/resumeapi/{routes.go,chain.go,writesafety.go,wireversion.go,errors.go,persist.go}`                                                | Route table and dispatch, write-safety kernel, production error vocabulary, persist helper (T4 plus phase integration)             |
