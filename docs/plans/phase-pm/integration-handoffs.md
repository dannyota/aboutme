...
Digest convention (T03, frozen): the stored 32-byte digest is SHA-256 over the
canonical raw spelling — prefix included for tokens (`amat_…`/`amrt_…`), bare 43
characters for codes — which domain-separates kinds. Consumers obtain digests
only through `NewToken`/`ParseToken`/`NewCode`/`ParseCode`, never by hashing
material themselves. A short entropy read surfaces the raw `io` error, which
endpoint tasks map to `server_error`.
