...
- [ ] Implement: cluster, capacity provider on Task 2's ASG, three services
      ...
      `http://127.0.0.1:2020/caddy-health` (Task 7) — a plain `wget` against the
      named production site block would 404 on Host mismatch, so the dedicated
      vhost is the named mechanism**; log groups per service with
      `var.log_retention_days`; Container Insights enabled for the task and
      resource metrics consumed by Task 9.
- [ ] Apply one runtime baseline in mocked tests and task definitions:
      ...
      with exact owner and mode prepared by Task 2 user data; Caddy gets
...
- [ ] Caddy task env additionally includes `CLOUDFRONT_ORIGIN_CIDRS` rendered
      ...
      execution role; bind mount Task 2's `/var/lib/caddy` and `/var/log/caddy`
      host volumes (D13).
- [ ] Inject Task 6's exact CloudFront distribution ID into Go as
...
- [ ] Wire private media into the server task: `MEDIA_BACKEND=s3`,
      `MEDIA_BUCKET` from Task 3, and `MEDIA_REGION=var.aws_region`. Leave
      ...
      credentials from the Task 4 server task role. `terraform test` proves that
