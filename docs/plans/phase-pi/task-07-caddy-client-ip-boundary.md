...
3. Strip every inbound forwarding header (`X-Forwarded-For`, `X-Real-IP`,
   ...
   `header_up`.
4. Route per `routes.caddy` (identical table to dev, including `/print/*` and
   `/internal-render/*` denial before the default web proxy); upstreams via
   `{$GO_UPSTREAM:server:8080}` / `{$WEB_UPSTREAM:web:3000}` env placeholders
   with dev defaults (D7).
