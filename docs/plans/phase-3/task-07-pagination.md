...
paper height as `min-height`. An ordinary page stays at the selected A4/Letter
height. A page containing an oversized block expands to
`2 × pageMarginY + optionalHeaderHeight + optionalHeaderBodyGap + max(mainColumnHeight, sidebarColumnHeight)`;
later pages flow after that computed box and never overlap or clip it.
...
axis (`geometry.md` §6; `print.md` §2 fixes the same 15 mm in `@page`). The
...
`--gap-entry` values: section-to-section, heading-to-first-entry, or entry-to-
entry respectively. Pagination adds a gap only when both adjacent blocks stay in
the same column and page; the first block pays zero. The rendered page uses the
returned `appliedGapBeforePx`, so measurement and slicing cannot disagree.
Internal `--gap-block` spacing is already inside the entry rect.
...
fails with the typed `pagination_measurement_required` error instead of emitting
an unpaged or empty document.
