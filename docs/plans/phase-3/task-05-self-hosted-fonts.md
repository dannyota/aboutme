...
**Tier:** Normal implementation. The integration owner performs the per-asset
license/provenance gate. The W4 phase review is the only fresh defect review.
This task owns no schema file; Task 5B releases document v2 after these bytes
are final.
...
inputs before `npm --prefix apps/web run build`. Its runtime stage copies the
complete `.output`, so no separate image-only license copy is allowed to drift.

## Steps

- [x] Record the reviewed font-catalog revision before fetching or generating an
      asset. Stop if it differs from this task's 26-row input matrix.
- [x] Write failing manifest-schema, path, hash, internal-name, license-file,
...
- [x] Fetch only the exact immutable inputs in `docs/design/fonts.md`. Verify an
...
- [x] Generate final WOFF2 assets with a pinned fonttools version only for
      ...
      hashes.
- [x] Commit `font-coverage.txt` as the standalone English, Vietnamese, and
      ...
      alone.
- [x] Verify the exact OFL file permits the chosen policy without a fee. Copy
      each exact license into its manifest-named public runtime path and
      generate `THIRD_PARTY_NOTICES.txt`. The integration owner rechecks every
      listed license hash, copyright notice, Reserved Font Name, and asset
      policy against the exact source before the catalog can land.
- [x] Emit local-only `@font-face` rules. Do not claim a face that the manifest
...
- [x] Implement `resolveFontSelection` and `fontsReady` exactly as above. Assert
...
- [x] Test that CSS has no remote URL, every asset hash and license matches,
      ...
      tree.
- [x] Run the focused asset and license checker
