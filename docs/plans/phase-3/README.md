...
| 4    | Independent sanitizer, renderer, and pagination suites      | High   | Partial     |
...
```mermaid
graph TD
    AP[Draft v4 and template contract approved] --> T5
    AP --> T6
    AP --> T8
    ...
    T5B --> T4A[T4 applyTemplate tests frozen]
    T4R --> T6[T6 renderer]
    T3 --> T6
    T2 --> T6
    T6 --> T4P[T4 pagination tests frozen]
    T4P --> T7[T7 pagination]
    T4A --> T8[T8 preset registry]
    T6 --> T9[T9 goldens]
    T7 --> T9
    T8 --> T9
...
Task 4 has five test-only freeze points. Its codegen-faithfulness suite is
...
and before T6. Its separate `applyTemplate` suite freezes after T5B and before
T8. T6 then passes its renderer-only gate with no pagination test in the tree. A
different fresh author freezes the still-failing pagination suite in a
pagination-only diff before T7. No blind author reads the implementation diff it
tests. The implementation authors fix findings; a fresh reviewer rechecks them.

Task 5B has its own high-risk chronology gate outside Task 4. Its two
independent font-v2/converter files freeze in a test-only diff after Task 5
finalizes the catalog and before the integration owner reads or writes the Task
5B implementation diff. Draft v4 approval must be recorded before T5, T6, or T8
starts.
