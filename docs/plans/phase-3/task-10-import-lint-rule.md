...
- [x] **Step 1: Failing rule test.** Using the `eslint` API (`ESLint` / `Linter`
      ...
      `crypto.getRandomValues()`, `crypto.randomUUID()`, every access through
      the `Intl` namespace (constructors and non-constructor entry points), and
      `toLocale*()` family, `fetch`, `$fetch`, `XMLHttpRequest`, `WebSocket`,
      and `EventSource` — assert each reports an error. Lint a clean
      renderer-style snippet (imports from `vue`, `@aboutme/schema`, sibling
