...
| P11-008 | A real browser performs one link and one reauthentication POST then follows the returned top-level URL; no privileged GET appears                     | Pinned `.mcp.json` procedure, Chrome version, accessibility snapshot, network log |
