...
| P1 auth           | **Implemented** — 11 of 11 tasks, all reviewed                    | Google/GitHub/LinkedIn OIDC+OAuth2 login, session lifecycle with FK-tracked rotation lineage, fail-closed CSRF, `/me` + session device management, explicit linking with email-merge rejection, web login/session pages. Six independent blind adversarial suites integrated (AC-AUTH-001…005, AC-SEC-002). Contract corrections recorded as DD-C1…DD-C17; schema head `00003_add_sessions_rotated_from`. **Exit gates not yet run** — final whole-branch review, adversarial review, UAT catalog + independent run, and Opus evidence verification remain |
...
**Why here, not P10:** P9A's staging gate can only exist if the infrastructure
...
> drops them from a diff. Adding the 3-resume trigger therefore requires
> extending `scripts/check-data-drift.sh` from a keyword-reject to a real
> cross-check for those classes, and broadening its keyword net (it currently
> misses `CREATE MATERIALIZED VIEW`, `CONSTRAINT TRIGGER`, and
> `TEMP`/`UNLOGGED`/`RECURSIVE` variants — review finding M-NEW). Until then the
> trigger cannot be migrated through the gate.
