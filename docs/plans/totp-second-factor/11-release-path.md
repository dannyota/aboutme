...
4007 activation, key rotation, production proof, and failure order. Read
...
- Modify `deploy/aws/scripts/deploy_test.sh` and
  `deploy/aws/scripts/testdata/respond` only for v0.4.7 floor and same-tag
...
- Modify `deploy/aws/scripts/fence.sh` only where the 4007 floor or the
...
- Extend the pre-lock refusal in `deploy.sh` that stops a revision turning
  ...
  `TOTP_ENROLLMENT_ENABLED=true` while the floor is below 4007, with a
  `deploy_test.sh` case.
...
- Prove the generic serialized activation accepts 4007 after 4002, never lowers
...
- Add `deploy/aws/scripts/deploy.sh --totp-key-reencrypt <tag>` as the supported
  ...
  4007 and the tag equal to the running app release, runs the conditional check
