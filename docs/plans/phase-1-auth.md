...
**Design:** [provider identity](../design/security.md#provider-identity),
[OAuth transactions](../design/security.md#oauth-transaction),
[sessions](../design/security.md#sessions), and the
[`users`/`identities`/`sessions` relational model](../design/data.md#relational-model).
...
## Design decisions this plan makes beyond the design

The [security design](../design/security.md) states the OAuth/session **policy**
precisely but leaves several **mechanisms** unspecified. Rather than leaving
TODOs, this plan makes an explicit call for each and flags it so Fable/Opus 5
can challenge it in review instead of discovering it mid-implementation:

| #   | Gap in the design                                                                                                                                                                                                                                                                                                                                                  | Decision made here                                                                                                                                                                                                           |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| 4   | ~~The API endpoint table doesn't list session endpoints~~ — FALSE PREMISE, corrected 2026-08-02: the [API endpoint design](../design/api.md#endpoint-groups) DOES list `GET /sessions`, `DELETE /sessions/{id}`, `DELETE /sessions` (rows added during P0 exit-gate design corrections, after this plan was drafted), with recent reauth on both revoke operations | Follow the design verbatim: `GET /api/v1/sessions`, `DELETE /api/v1/sessions/{id}`, `DELETE /api/v1/sessions` (Task 9) — implementer adds these to `docs/api/openapi.yaml`; the invented `POST /sessions/revoke-all` is void |
...
No acceptance ID — the
[OAuth transaction design](../design/security.md#oauth-transaction) is normative
but has no row in `traceability.md`; flag this to the integration owner as a
traceability gap to backfill.
...
- Produces HTTP surface (design decision 4 — none of these paths existed in the
  drafting-time [API endpoint table](../design/api.md#endpoint-groups)):

  | Method + path                  | Auth                           | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                |
  | ------------------------------ | ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
  | `GET /api/v1/me`               | session                        | `{data:{user, csrfToken, identities:[{provider}]}}` — CSRF token **only** here, in the body, never cookie/URL/log ([security design](../design/security.md#csrf-and-canonical-origin) verbatim)                                                                                                                                                                                                                                        |
  ...
  | `DELETE /api/v1/sessions/{id}` | session + CSRF + recent reauth | revoke one session (must belong to caller — 404, not 403, if it belongs to someone else, to avoid confirming the id exists); revoking the **current** session also clears its cookie in the response. (Corrected 2026-08-02: the [API endpoint design](../design/api.md#endpoint-groups) requires recent reauth on per-session revoke; the earlier session+CSRF-only row contradicted it — design wins)                                |
  | `DELETE /api/v1/sessions`      | session + CSRF + recent reauth | logout-everywhere: `SessionManager.RevokeAll`; clears current cookie + `Clear-Site-Data`. (Corrected 2026-08-02: the [API endpoint design](../design/api.md#endpoint-groups) lists `DELETE /sessions` — the previously invented `POST /sessions/revoke-all` rested on the false premise that the design omitted these endpoints; the session-endpoint rows were added during the P0 exit-gate corrections after this plan was drafted) |
...
| DD-C11  | Both `DELETE /sessions/{id}` and `DELETE /sessions` require session + CSRF + recent reauth; logout-everywhere is `DELETE /sessions`                                        | [API endpoint design](../design/api.md#endpoint-groups) requires reauth on both revokes; the plan's invented `POST /revoke-all` rested on a false premise |
