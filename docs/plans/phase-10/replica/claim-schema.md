...
- [ ] Add `TestRuntimeSharedClaimSchemaObjectsExist` using the existing
...
- [ ] Before writing migration 17, run from `apps/server` and record the four
...
- [ ] Follow the version-16 protected owner pattern. The first Up statement
      ...
      generation once.
- [ ] Fix policy IDs, kinds, running/waiting limits, queue flags and deadline
...
- [ ] Add summary keys, catalog foreign keys, 32-byte digests, nonnegative
...
- [ ] Add parents with non-nil claim and replica IDs, exact policy/work shape,
...
- [ ] Add scopes with parent/policy and summary foreign keys, request ordinals
...
- [ ] Recompute the request digest from normalized rows with a fixed owner
...
- [ ] Schedule deferred whole-record assertions on every parent, scope or
...
- [ ] Preserve terminal release receipts. A child cannot disappear while its
...
- [ ] Attach ordinary write-entry assertions to all four tables. Reject
...
- [ ] Add each negative test before its constraint. Complete valid fixtures must
      fail on the intended SQLSTATE and named constraint or assertion; an
      unrelated error is not evidence.
- [ ] Test each required and forbidden parent field separately, including SQL
...
- [ ] Pin the published 165-byte SSE vector and literal digest independently in
...
- [ ] Prove every fixed catalog row and reject drift. Exercise each running and
...
- [ ] Prove C05 IP/account insertion and release are atomic. Missing, extra,
      swapped, duplicate or cross-parent scopes fail. Denied or rolled-back
      fixtures leave no partial count or child.
- [ ] Prove allocation ordinal uniqueness within a scope, independent request
...
- [ ] Prove waiting/running rows cannot be deleted, released children are
...
- [ ] Use pinned independent connections for duplicate claim/allocation
...
- [ ] Exercise every real named login for denied direct DML and helper
...
- [ ] Upgrade a populated version-16 fixture without changing membership,
...
- [ ] Inspect the owned changes and report exact red/green evidence, object and
