...
- [ ] Add a bounded live existence test for the six exact signatures. Observe
...
- [ ] Add failing state, role, replay and malformed-input cases before each
...
- [ ] Enter runtime_begin_migration_write('migration-00022') first and finish
...
- [ ] Use runtime-owner SECURITY DEFINER functions, search_path pg_catalog,
...
- [ ] Validate every scalar, nullable branch and source bound before mutation.
...
- [ ] Lock public_state, capacity and the operation parent before action rows,
...
- [ ] Infer fresh workflow only through the accepted first-action graph. Finish
...
- [ ] For replay, compare the supplied canonical argument digest and retained
...
- [ ] Install canonical helpers and compute all ten published vectors from
...
- [ ] Sample runtime_sample_lifecycle_time once after needed locks. Clamp to the
...
- [ ] Lock all 48 rate-partition rows in partition/policy order when an action
...
- [ ] Prepare scale-out requires one active serving replica, desired one,
...
- [ ] Initial and booked first serving activation require desired one, zero
...
- [ ] Maintenance activation requires its completed maintenance wake and zero
...
- [ ] Prepare scale-in selects an exact active serving target at desired two
...
- [ ] Termination records one immutable request/identity/reason intent and
...
- [ ] Cover every action/workflow branch and every required predecessor. Reject
...
- [ ] Build valid owner fixtures with corrupt retained result hashes/shapes and
...
- [ ] Exercise all input bounds, nullable inputs, reason/state combinations,
...
- [ ] Prove one and two fenced replacements, consume-once predecessor and final
...
- [ ] Test all-24 partition consistency and exact disabled/enabled branches.
...
- [ ] Pin future timestamps and generation boundaries. Use only the accepted
...
- [ ] Connect as every real named login. Prove lifecycle success and every
...
- [ ] Use independent pinned pools and observed lock waits for competing fresh
...
- [ ] Upgrade a populated version 21 fixture explicitly to 22. Preserve
...
- [ ] Add six queries in the owned SQL source. Each uses one MATERIALIZED
...
- [ ] Request root generation when SQL/query sources are ready. Inspect the
      resulting parameter and row types before dependent store implementation.
      Root owns all generated files and shared changes.
- [ ] Write failing tests for six method mappings and all result fields.
      ...
      callback.
- [ ] Use one WriteTxRunner call and one generated query per method. Return
...
- [ ] Prove real one-call execution, rollback versus committed state, result
...
- [ ] Report exact red/green evidence, action/role/replay/lock coverage, changed
