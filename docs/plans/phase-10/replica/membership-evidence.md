...
- [x] Before migration 23 exists, observe a bounded test report both absent
...
- [x] Add failing identity, replay, role and cleanup cases before implementing
...
- [x] Enter runtime_begin_migration_write('migration-00023') first and finish
      last. Down is inert. Keep migration 00015 unchanged.
- [x] Require runtime_fencing_proofs empty before changing its schema. No proof
...
- [x] Add request_id text NOT NULL, printable ASCII 1..128 bytes and UNIQUE. Add
...
- [x] Test a populated version 22 upgrade with ordinary business, membership,
...
- [x] Use fixed serving/maintenance wrappers over an owner-only helper. Validate
...
- [x] Require the exact tuple/kind and prepared draining target, no termination
...
- [x] Bind operation_id and expected controller generation to the matching
...
- [x] Insert one immutable zero-count receipt, change draining to left and
...
- [x] Exact receipt replay returns the retained result without new live
...
- [x] Require direct fencing-proof role, exact identity, observed_state exactly
      ...
      database loss.
- [x] Lock public_state, capacity, target replica, matching intent/proof, then
...
- [x] Store request_id with or without an intent and enforce uniqueness among
...
- [x] A left target stays left and gains audit proof with reclaimed count zero.
...
- [x] Process all live owned claims in one transaction, with ordered locks and a
...
- [x] After needed locks, sample runtime_sample_membership_time once and clamp
...
- [x] Exact proof replay compares retained input identity and returns stored
...
- [x] Cover each leave predicate, historical generation equal/greater, wrong
...
- [x] Cover every proof source state, left audit-only behavior, exact intent, no
...
- [x] Exercise multiple claim policies/scopes and waiting/running/released
...
- [x] Use independent pinned pools with observed lock waits for claim acquire
...
- [x] Prove all real named role success/denial, exact owners/search paths/ACLs,
      no helper or direct table access, no supplied values in fixed errors and
      hostile search-path resistance.
- [x] Test future-time clamp and generation bounds with the isolated sampler
...
- [x] Add three exact one-call MATERIALIZED queries and explicit scalar casts.
...
- [x] Request root sqlc generation after SQL/query sources are ready. Inspect
...
- [x] Write failing store tests for method mapping, every required result field,
...
- [x] Prove real one-call evaluation, NULL rejection for each result attribute,
...
- [x] Report exact red/green evidence, role/replay/lock/cleanup proof, changed
