...
| Dates           | `Mon YYYY`                        | "Mar 2021" needs no field-order inference; "03/2021" is unambiguous only because 2021 > 12. The renderer uses fixed English and Vietnamese month tables (`contract.md` §5.4), selected by `resumes.lng`.                                                                                                                                                                                              | OBSERVED (the string), INFERRED (parser behaviour) |
...
| -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| "A non-standard font breaks ATS"             | Not assumed either way. Font tests cover text extraction and embedding per family. This preset chooses legibility; a failed extraction test blocks that asset from an ATS claim, not from the user catalog.                |
...
| Apply keeps `pageFormat` ([limitations item 2](../limitations.md))           | A preset's A4 value seeds a new resume; switching templates keeps the owner's paper size.                                                                                                                                               |
