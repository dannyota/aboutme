...
WCAG 2.1 before the clamp (`contract.md` §8), roles per `colors.md` §4.
...
- **Small caps and a separate italic token**: the renderer requests only its
  fixed 400/700 roles and exposes no style token (`tokens.md` §3.2), so an
  italic employer line is unreachable even when the family provides italics.
