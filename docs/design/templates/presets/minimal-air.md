...
reads hard. Links fall back to `primary` (`colors.md` §4), so they are ink
...
`colors.md` §4's 25 % mix toward the surface as an sRGB-channel mix; in linear
...
- A photo ([limitations item 3](../limitations.md)) and a section `iconKey`
  ([item 5](../limitations.md)) still render; `header.iconStyle` governs the
  contact row only. Either puts the one graphic mark on a mark-free page.
