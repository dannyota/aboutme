...
1. **One ink to read, one to navigate by.** `colors.text` is a warm near-black;
   ...
   (`colors.md` §4). Hierarchy _is_ the colour, not a highlight over one.
...
scores 3.91:1 and loses. The outline fallback of `colors.md` §4 cannot fire here
— or anywhere: the better of black and white is never worse than 4.58:1. Base 15
px (11.25 pt) sits above the 13 px legibility discussion of `colors.md` §5; at
...
- `iconKey` has no global off switch ([limitations item 5](../limitations.md)),
  so section icons render beside the coloured headings whatever the preset
  wants.
