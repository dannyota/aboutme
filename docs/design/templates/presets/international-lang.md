...
- **`font.family` is a choice, not a coverage promise.** The catalog measures
  final assets and labels each family. It does not reject a family because a
  script or face is absent.
- Be Vietnam Pro remains this preset's default because it is a Vietnamese-led
  family and the final asset must be measured against the committed Vietnamese
  exemplar. The renderer uses the bundled sans fallback for a missing glyph.
- Content outside the declared English, Vietnamese, and renderer punctuation set
  is accepted. Its platform-fallback boundary is visible in the editor.
...
`colors.md` §4. Every floor is met with no clamping. Track is accent 80% toward
