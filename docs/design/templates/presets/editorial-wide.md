...
4. **A named identity.** `customization` stores no template id
   ([limitations item 1](../limitations.md)): after apply, nothing separates
   these values from a user who reached them by hand — which is, in the end, the
   point.
