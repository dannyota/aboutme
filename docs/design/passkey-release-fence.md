...
v0.4.5 (numeric 4005), and the tag to equal the running app release. It runs the
...
`deploy.sh` to `TOTP_ENROLLMENT_ENABLED` with floor v0.4.5 (numeric 4005). A
...
while the item is missing or below 4005. Before acquiring the lock, `deploy.sh`
refuses to register an app revision that turns TOTP enrollment on while the item
is missing or below 4005, as it refuses passkey enrollment below 4002. Applying
...
recovery codes while TOTP stays active. Activation for v0.4.5 raises the fence
to 4005 under the same lock before the flag turns on.
