...
sequenceDiagram
  ...
  G-->>B: signed view token, proof-of-work challenge, mode, consent state
...
| 5   | Signed one-time token  | Issued by start, HMAC-signed, bound to resume and mode; accepted 8 s to 30 min after issue, once                                                                                                     | `invalid`                               |
...
1. `AWSManagedRulesBotControlRuleSet`, Common level, rule group override Count,
   scoped down to `POST` requests whose path starts with
   `/api/v1/public/views/collect`.
...
`x-amzn-waf-aboutme-dc` to Go and removes every other `x-amzn-waf-*` header. Go
reads them only from the loopback proxy, as it reads `X-Real-IP`. Any other
listener, including a future vCDN listener, removes them. Without an edge that
sets them, layers 2 and 3 are off and the rest still apply.
...
- `token`: base64url of
  `resumeRef (8 bytes) ‖ mode (1) ‖ issuedAt (8) ‖ nonce (16) ‖ HMAC-SHA-256 tag (32)`.
  The key is 32 random bytes made at process start and never stored, so a deploy
  ends outstanding tokens; those views are lost, not double counted.
- `challenge`: an ALTCHA v2 challenge from
  `github.com/altcha-org/altcha-lib-go/v2` (MIT), algorithm `PBKDF2/SHA-256`,
  HMAC-signed with a second per-process key, expiring with the token ([ALTCHA
  Go][altcha-go]).
...
cost targets a median of at most 1 s on a mid-range 2021 Android phone; the
backend measures it and records the parameter in [budgets](../budgets.md). No
third party is contacted: ALTCHA's hosted Sentinel is not used.

Collect verifies, in order: body shape, token tag, token age within 8 s to 30
min, nonce unused, challenge signature and solution, then the edge headers. A
used-nonce set in memory holds each nonce until its token expires. Any failure
records `invalid`, except a malformed body, which is a 400 and records nothing.
...
- **Owner:** start and collect read the `__Host-session` cookie read-only. When
  the session's account owns the resume, collect records nothing and the network
  key is marked `owner` for the rest of the day, so the owner's later signed-out
  views from the same network are not counted either. The response tells the
  owner's own script that it is the owner, so no popup shows.
...
and flushes it every 60 s and at graceful shutdown with one upsert per cell into
`resume_view_days` and `resume_share_signal_days`
([delivery](delivery.md#schema)). A crash loses at most 60 s of counts. A resume
that is unpublished, deleted, or gone between start and collect records nothing:
collect re-checks the live gate. Days use the Asia/Ho_Chi_Minh zone.

## What the owner sees

The Views page (`/app/views`) lists every resume that has been published, with
its counts for the last 7, 30, and 90 days, and links to a resume page
(`/app/views/{id}`) with:
...
The page reads only aggregates. In `ask` and `sign_in` modes the same resume
page also lists recorded views ([tracking](tracking.md#what-the-owner-sees)).
