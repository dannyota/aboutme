...
The chrome is neutral: the zinc palette, Inter, and emerald only for saved and
active states. The resume preview on its muted canvas is the one element with
visual weight. Copy is sentence case, buttons name the action they perform, an
empty state says what to do next, and an error says what happened and how to fix
it.
